...
def main():
    ...
    if env_type == 'T':
        ...
        agent = QLearningAgent(env, 0.2, 0.9, 0.8, 500) if model.lower(
        ) == 'q' else SARSALearningAgent(env, 0.2, 0.9, 0.8, 100)
        agent.train()
        ...
        agent = QLearningAgent(env) if model.lower(
        ) == 'q' else SARSALearningAgent(env)
        file = 'data/sarsa_learning.bson' if model.lower() == 's' else 'data/q_learning.bson'
        agent.load(file)
